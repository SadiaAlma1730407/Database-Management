...
<body>
  <div class="wrapper">
      <div class="sidebar">
        ...
        <ul>
          ...
          <li style="background-color: #594f8d;"><a href="course_report.php"><i class="fas fa-project-diagram"></i>Course Report</a></li>
  ...
  <?php
    ...
    $fire_course = mysqli_query($con, $sql_course);
    while ($result_course = mysqli_fetch_array($fire_course)) {
     $CourseID = '"'.$result_course['CourseID'].'"';
      $barchartdata .="[".$CourseID.",";
      ...
      while ($result_semester = mysqli_fetch_array($fire_semester)) {
        ...
        while ($result_id = mysqli_fetch_array($fire_id)) {
          ...
          $sql = '
          SELECT 
              student.StudentID ,course.CourseID,(evaluation.Obtainedmarks / assessment.Marks)*100 AS FinalMark , semester.SemesterName
          FROM
              evaluation
          INNER JOIN
          ...
          INNER JOIN
              section ON section.SectionID = assessment.SectionID
               INNER JOIN
                 course ON course.CourseID = section.CourseID
          INNER JOIN
              semester ON semester.SemesterID = section.SemesterID
          WHERE semester.SemesterName = '.$SemesterName." AND student.StudentID = ".$StudentID." AND course.CourseID = 
          ".$CourseID;
...
<script type="text/javascript">
  ...
  function drawChart() {
    ...
    var options = {
      chart: {
        title: 'course wise Student Performance',
